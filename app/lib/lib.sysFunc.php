...
function do_cooccuring_wc_job($filename)
{
	...
	file_put_contents("output/fileprocess.txt", $filename);

	//exec("sh scripts/mrjob.sh ".$filename);
}
...
{
	$qry_url = "https://www.googleapis.com/language/translate/v2/detect?key=".GOOGLE_TRANSLATE_API_KEY."&q=".urlencode($qrystr);
...
{	
	$qry_url = "https://www.googleapis.com/language/translate/v2?key=".GOOGLE_TRANSLATE_API_KEY."&source=".$lang_src."&target=".$lang_dest."&q=".urlencode($qrystr);
